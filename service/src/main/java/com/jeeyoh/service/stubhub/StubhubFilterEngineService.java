				...
				for(StubhubEvent stubhubEvent : stubhubEventsList){
					...
					if(eventDate != null)
					{
						...
						for(int j = 0; j < weekendList.size(); j++){
							...
							logger.debug("eventDate:  "+eventDate + "weekendList::  "+weekendList.get(j));
							/*if((eventDate.compareTo(currentDate) >= 0) && (eventDate.compareTo(sdf.parse(sdf.format((Date)weekendList.get(j)))) == 0))
							{*/
							if((eventDate.compareTo(weekendList.get(j)) == 0))
							{
								count1++;
								...
								if(isPageExist == null)
								{
									...
									//logger.debug("Page After Save :::"+page1);
									events.setPage(page);
								}
								else
								{
